...
def main():
    ...
    testing_configs = load_conf(r"C:\courses_and_personal_projects\hardware_data_pipeline\conf\occt_software.yml")
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        while time.time() < end_time:
            ...
            mainmemory.payload_buffer_add(filename=r"C:\courses_and_personal_projects\hardware_data_pipeline\conf\occt_software.yml", cpu_payload=cpu_data, gpu_payload=gpu_data, memory_payload=memory_data)

            check_buffer_result = mainmemory.check_buffer(buffer_size_limit=200000)

            upload_blob_result = upload_blob(credentials=credential, account_url=account_url, buffer_complete=check_buffer_result, sensor_buffer=mainmemory.payload)
